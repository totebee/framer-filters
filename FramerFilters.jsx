...
export function FramerFilters({ imageUrl }) {
  ...
  const isCustomizable = filter.name === "Custom" || filter.name === "Blur" || filter.name === "Brightness" || filter.name === "Contrast" || filter.name === "Saturation" || filter.name === "Hue Rotate" || filter.name === "Sepia" || filter.name === "Grayscale" || filter.name === "Invert" || filter.name === "Opacity";
    ...
    .map(([k, v]) => {
      ...
      return (
        <div key={k} style={{ margin: "8px 0" }}>
          <label style={{ fontSize: 13 }}>{config.label}: {value}{config.unit}</label>
          <input
            type="range"
            min={config.min}
            max={config.max}
  ...
  return (
    <div style={{ width: 400, margin: "0 auto", fontFamily: "Inter, sans-serif" }}>
      ...
      <div style={{ position: "relative", width: 320, height: 240, margin: "0 auto", borderRadius: 12, overflow: "hidden", boxShadow: "0 2px 16px rgba(0,0,0,0.08)" }}>
        <img
          src={imageUrl}
          alt="Preview"
          style={{
        ...
        <div style={{ marginTop: 18, textAlign: "center" }}>
          <div style={{ marginBottom: 8 }}>
            ...
            <button
              ...
              style={{ padding: "6px 12px", borderRadius: 4, border: "none", background: "#222", color: "#fff", fontWeight: 500, cursor: "pointer" }}
            >
              Save Preset
            </button>
          </div>
